function [kNeighborsIndex, IndexOfCases, SolutionReuse, SolutionMajorityRule, SolutionProbabilisticMethod, SolutionClassBasedMethod, MinimumSolSim, MoySolSim, StdSolSim]=ModelCBRdlaTable6(CaseCBR, QueryCase, Casestab, kNeighbors)
global w;
%%
%%Data loading
step=0.1;
...
%for alpha=0.1:step:0.9
   ...
   % QueryCase=[1.4 1 3 3.2 3 2 4.1 5 1 7];
    ...
    mdl = fitcknn(X,Y, 'NumNeighbors',kNeighbors);
    alpha; 
    flwr = QueryCase; % the case to be predicted
    flwrClass = predict(mdl,flwr)
        %%
   %MON KNN
   training=CaseCBR2(:,1:n+1);%j'inclus meme la colonne des solution du training
   training_labels=idx;
   [predicted_label, kNeighborsIndex, kNeighborsSol, MinimumSolSim, MoySolSim, StdSolSim]=KNN_Customized(kNeighbors,training,training_labels,QueryCase);
   save('KNN_Data.mat','predicted_label','kNeighborsIndex', 'kNeighbors');
    ...
    v=1;
    %clear mat;
    %La variable ligne du tableau 6 colonne 2 IndexOfCases
    IndexOfCases='';
    for i=1:(length(kNeighborsIndex)-1)
        IndexOfCases=[IndexOfCases num2str(kNeighborsIndex(i)) ', '];
        CaseSim(v,:)=CaseCBR2(kNeighborsIndex(i),1:n);
        SolCaseSim(v)=CaseCBR2(kNeighborsIndex(i),1+n);%%NEWWWWWWWWWW%%
        mat{1,v}=['Case' num2str(CaseCBR2(kNeighborsIndex(i),n+2)) ];
        %IndexOfCases=[IndexOfCases num2str(CaseCBR2(i,n+2)) ', '];
        v=v+1;
    end
        ET='and '
        if(v==1)
        ...
        i=length(kNeighborsIndex);
        IndexOfCases=[IndexOfCases ET num2str(kNeighborsIndex(i))];
        CaseSim(v,:)=CaseCBR2(kNeighborsIndex(i),1:n);
        SolCaseSim(v)=CaseCBR2(kNeighborsIndex(i),1+n);%%NEWWWWWWWWWW%%
        mat{1,v}=['Case' num2str(CaseCBR2(kNeighborsIndex(i),n+2)) ];
    
        %%
    disp(['Pour k=' num2str(kNeighbors)  ' et alpha=' num2str(alpha)  ' les cas les plus similaires du QueryCase sont:'])
    mat
